...
OPTIMIZE_PROMPT = '''
    ...
    ** 리뷰할 만한 충분한 코드가 없다면 억지로 점수를 내지 말고 "리뷰할 내용이 없습니다"라고 "review"에 담아 반환한다.
    ...
    - **review**: 성능 최적화 관점에서 문제점과 4가지 분석 기준("시간복잡도", "공간복잡도", "알고리즘 적합성", "자료 구조 적합성") 관점에서 문제점을 찾고, 개선된 코드 스니펫을 제공한다.
...
BASIC_PROMPT = '''
    ...
    ** 가독성과 안정성을 저해하는 요소를 지적하고, 4가지 분석 기준("코드 가독성", "에러 처리", "코드 주석 작성", "테스트 케이스 작성") 관점에서 문제점을 찾고, 개선된 코드 스니펫을 제공한다.
...
NEWBIE_PROMPT = '''
    ...
    ## 규칙
    ** 초보자가 이해할 수 있도록 쉬운 용어와 설명을 사용한다. 4가지 분석 기준("기본 자료형 사용", "조건문 작성", "반복문 작성", "함수 정의 연습") 관점에서 문제점을 찾고, 개선된 코드 스니펫을 제공한다.
    ** 출력은 단 한 번만 한다.
...
STUDY_PROMPT = '''
    ...
    ## 규칙
    ** 학습자의 성장에 도움이 되는 피드백을 제공한다. 4가지 분석 기준("문제 해결 전략", "최적화 가능성 분석", "알고리즘 설계", "데이터 구조 선택") 관점에서 문제점을 찾고, 개선된 코드 스니펫을 제공한다.
    ** 출력은 단 한 번만 한다.
