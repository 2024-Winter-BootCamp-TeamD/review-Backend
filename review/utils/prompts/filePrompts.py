CLEAN_PROMPT = '''
    ...
    # 클린 모드
    - 분석 기준:
        1. 명명규칙 (함수, 변수, 클래스의 이름이 역할과 의도를 명확히 표현하고 있는가?, 축약형 또는 의미 없는 이름이 사용되고 있는가?)
        2. 재사용성 (동일하거나 유사한 코드가 반복적으로 사용되고 있지는 않은가?, 모듈화와 함수화가 제대로 이루어졌는가?)
        3. 단일책임원칙 (함수 또는 클래스가 하나의 역할만 수행하고 있는가?, 과도한 역할이 하나의 함수와 클래스에 몰려있지는 않은가?)
        4. 캡슐화 (객체의 속성은 적절히 숨겨지고 필요한 부분만 노출되고 있는가?, 직접적인 속성 접근을 제한하고 getter와 setter를 사용하는가?)
...
OPTIMIZE_PROMPT = '''
    ...
    # 최적화 모드
    - 분석 기준:
        1. 시간복잡도 (알고리즘이 문제의 크기에 따라 얼마나 빨리 실행 시간이 증가하는가?, 중복 연산 제거와 효율적인 반복문 설계가 이루어졌는가?)
        2. 공간복잡도 (데이터 구조나 변수를 과도하게 생성하여 메모리가 낭비되는가?, 알고리즘이 사용하는 메모리가 입력 크기 증가에 따라 효율적으로 관리되는가?)
        3. 알고리즘 적합성 (문제의 요구사항을 충족하는 가장 최적의 알고리즘이 선택되었는가?, 구현된 알고리즘이 실제 데이터 특성을 효과적으로 반영하고 있는가?)
        4. 자료 구조 적합성 (사용된 자료 구조가 데이터 접근과 수정과 삭제 작업에 효율적인가?, 선택된 자료 구조가 문제 해결에 최적화되어 있는가?)
